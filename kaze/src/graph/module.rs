...
impl<'a> Module<'a> {
    ...
    /// ```
    pub fn input<S: Into<String>>(&'a self, name: S, bit_width: u32) -> &Input<'a> {
        let name = name.into();
    ...
    /// ```
    pub fn mux<S1: Into<&'a Signal<'a>>, S2: Into<&'a Signal<'a>>, S3: Into<&'a Signal<'a>>>(
        &'a self,
        cond: S1,
        when_true: S2,
        when_false: S3,
...
impl<'a> Input<'a> {
    ...
    // TODO: Rename i?
    pub fn drive<S: Into<&'a Signal<'a>>>(&'a self, i: S) {
        let i = i.into();
...
impl<'a> From<&'a Input<'a>> for &'a Signal<'a> {
    fn from(i: &'a Input<'a>) -> Self {
        i.value
    }
}

impl<'a> From<&'a Output<'a>> for &'a Signal<'a> {
    fn from(o: &'a Output<'a>) -> Self {
        let parent = o.data.module.parent.expect("TODO better error pls");
        o.data.module.context.signal_arena.alloc(Signal {
...
impl<'a> From<&'a Register<'a>> for &'a Signal<'a> {
    fn from(r: &'a Register<'a>) -> &'a Signal<'a> {
        r.value
    }
}
...
macro_rules! op_impl {
    ($($t:tt)*) => ($(
        impl<'a, S: Into<&'a Signal<'a>>> Add<S> for &'a $t<'a> {
            type Output = &'a Signal<'a>;

            fn add(self, rhs: S) -> Self::Output {
                let lhs: &'a Signal<'a> = self.into();
                let rhs: &'a Signal<'a> = rhs.into();
                lhs + rhs
            }
        }

        impl<'a, S: Into<&'a Signal<'a>>> BitAnd<S> for &'a $t<'a> {
            type Output = &'a Signal<'a>;
        ...
        impl<'a, S: Into<&'a Signal<'a>>> BitOr<S> for &'a $t<'a> {
            type Output = &'a Signal<'a>;
        ...
        impl<'a, S: Into<&'a Signal<'a>>> BitXor<S> for &'a $t<'a> {
            type Output = &'a Signal<'a>;
        ...
        impl<'a, S: Into<&'a Signal<'a>>> Mul<S> for &'a $t<'a> {
            type Output = &'a Signal<'a>;
        ...
        impl<'a, S: Into<&'a Signal<'a>>> Shl<S> for &'a $t<'a> {
            type Output = &'a Signal<'a>;
        ...
        impl<'a, S: Into<&'a Signal<'a>>> Shr<S> for &'a $t<'a> {
            type Output = &'a Signal<'a>;
        ...
        impl<'a, S: Into<&'a Signal<'a>>> Sub<S> for &'a $t<'a> {
            type Output = &'a Signal<'a>;
...
pub(crate) struct InputData<'a> {
    ...
    pub bit_width: u32,
    // TODO: Rename?
    pub driven_value: RefCell<Option<&'a Signal<'a>>>,
}
